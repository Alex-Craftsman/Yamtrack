"""Release approval helpers for Seerr/Radarr requests."""
...
def title_aliases(movie: dict[str, Any]) -> list[str]:
    """Return normalized requested/alternate titles for a Radarr movie."""
    titles = [
        movie.get("title"),
...
def seerr_requests() -> list[dict[str, Any]]:
    """Fetch recent movie requests from Seerr."""
...
def score_release(movie: dict[str, Any], release: dict[str, Any]) -> ReleaseScore:
    ...
    if release.get("rejected"):
        score -= 200
        warnings.append("Radarr rejected this release")
