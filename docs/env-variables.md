...
| `POSTER_CACHE_DIR` | Local filesystem directory for cached external posters. Defaults to the repository `poster` directory locally and `/poster` in Docker.                                                                                                             |
| `POSTER_CACHE_TIMEOUT_HOURS` | Number of hours before a cached poster is refreshed in the background. Defaults to `48`.                                                                                                                                               |
| `POSTER_CACHE_MAX_CONCURRENT_DOWNLOADS` | Maximum number of external poster downloads handled by the app at once. Extra cache misses are redirected to the original image URL instead of waiting. Defaults to `5`.                                                   |
| `GUNICORN_WORKERS` | Number of Gunicorn worker processes. Defaults to `min(cpu_count * 2 + 1, 4)`.                                                                                                                                                           |
| `GUNICORN_THREADS` | Number of threads per Gunicorn worker. Defaults to `4`.                                                                                                                                                                                |
